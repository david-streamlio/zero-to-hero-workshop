...
public class CoinbaseTickerTracker implements Function<Ticker, Void> {
    ...
    public Void process(Ticker input, Context context) throws Exception {
        ...
        try {
            ...
            st.setTimestamp(1, new Timestamp(input.getMillis()));
            st.setFloat(2, input.getPrice());
            st.setString(3, input.getProduct_id());
